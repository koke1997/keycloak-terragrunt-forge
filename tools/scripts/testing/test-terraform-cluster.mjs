...
console.log('🔧 Testing Keycloak to Terraform Conversion - Fixed Version');
console.log('===========================================================');
...
function keycloakRealmJsonToTerragrunt(json, fileName) {
    ...
    // Provider configuration for submodules
    files.push({
        filePath: `${realmDir}/providers.tf`,
        content: generateProvidersConfig()
    });
    ...
    files.push({
        filePath: `${realmDir}/terragrunt.hcl`,
        content: generateTerragruntConfig(realm)
    });
...
resource "keycloak_realm" "realm" {
  ...
  ${json.accessTokenLifespan ? `access_token_lifespan = "${json.accessTokenLifespan}s"` : ''}
  ${json.ssoSessionIdleTimeout ? `sso_session_idle_timeout = "${json.ssoSessionIdleTimeout}s"` : ''}
  ${json.ssoSessionMaxLifespan ? `sso_session_max_lifespan = "${json.ssoSessionMaxLifespan}s"` : ''}
}
...
function generateTerragruntConfig(realm) {
    return `# Terragrunt configuration for realm: ${realm}
terraform {
  source = "."
}
...
# Input values for the realm
inputs = {
  groups = [
    # Groups will be populated from realm.json
  ]

  users = [
    # Users will be populated from realm.json
  ]

  roles = {
    # Roles will be populated from realm.json
  }

  clients = [
    # Clients will be populated from realm.json
  ]
}`;
}
...
function addGroupsModule(files, realmDir, groups, realm) {
    ...
    files.push({
        filePath: `${realmDir}/groups/providers.tf`,
        content: generateProvidersConfig()
    });
}
...
function addUsersModule(files, realmDir, users, realm) {
    ...
    files.push({
        filePath: `${realmDir}/users/providers.tf`,
        content: generateProvidersConfig()
    });
}
...
function addRolesModule(files, realmDir, json, realm) {
    ...
    files.push({
        filePath: `${realmDir}/roles/providers.tf`,
        content: generateProvidersConfig()
    });
}
...
function addClientsModule(files, realmDir, clients, realm) {
    ...
    files.push({
        filePath: `${realmDir}/clients/providers.tf`,
        content: generateProvidersConfig()
    });
}
...
resource "keycloak_openid_client" "clients" {
  ...
  access_token_lifespan = each.value.accessTokenLifespan

  # Client authentication
  client_authenticator_type = each.value.clientAuthenticatorType
}`;
}
...
async function testConversion() {
    ...
    for (const file of testFiles) {
        ...
        try {
            ...
            // Save the results
            const outputDir = './terraform-cluster-output';
    ...
    console.log('   - Provider configurations added to each module');
    console.log('   - Terragrunt configuration included for deployment');
    console.log('   - Files ready for cluster deployment');
}
